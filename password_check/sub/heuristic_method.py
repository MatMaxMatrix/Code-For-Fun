...
def suggerimenti_euristici(password: str, esito: bool, messaggio: str, in_lista_vietate: bool, simile_a_vietate: bool):
    """
    ...
    if simile_a_vietate:
        suggerimenti_list.append("Attenzione! Evita password comuni o simili a password compromesse")
    
    return suggerimenti_list

def analizza_password_euristica(password: str):
    """
    ...
    if not password.strip():
        ...
        # Metodo Euristico
        esito, messaggio = analisi_euristica(password, pwd_vietate)
        in_lista_vietate = password.lower() in pwd_vietate
        
        # Verifica se è simile a una password vietata
        simile_a_vietate = False
        if not in_lista_vietate and len(password) >= 8:  # Solo se non è già nella lista e ha lunghezza sufficiente
            threshold = 0.8
            for pwd in pwd_vietate:
                if similar(password.lower(), pwd) >= threshold:
                    simile_a_vietate = True
                    break
    
    # Genera suggerimenti
    suggerimenti = suggerimenti_euristici(password, esito, messaggio, in_lista_vietate, simile_a_vietate)
...
if __name__ == "__main__":
    ...
    for password in test_passwords:
        esito, messaggio, suggerimenti = analizza_password_euristica(password)
    ...
    esito, messaggio, suggerimenti = analizza_password_euristica(password_input)
    print(f"Esito: {'Sicura' if esito else 'Debole'}")
    print(f"Messaggio: {messaggio}")
    print(f"Suggerimenti:")
    for suggerimento in suggerimenti:
